...
/**
 * occured during plugin activation
 */

 class Vxpt_Activator
 {
...
public static function activate(){
    ...
    if (function_exists('is_multisite') && is_multisite()){
        ...
        $blogids = $wpdb->get_col("SELECT `blog_id` FROM $wpdb->blog ");
...
/**
     * creation of tables while activate plugin
     * @throws Exception
     */
private function create_tables(){
    ...
    //vxpt_templates
    $table_name = $wpdb->prefix . 'vxpt_templates';
    ...
    if($wpdb->get_var("show tables like '{$table_name}'") != $table_name) {
        ...
        $sql = "CREATE TABLE IF NOT EXISTS " . $table_name . "(
            ...
            $wpdb->insert($table_name, ['template_name' => 'demo', 'style' => 'demo.css', 'html' => 'demo.html', 'image' => 'demo.jpeg', 'created_at' => $now, 'updated_at' => $now]);
            $wpdb->insert($table_name, ['template_name' => 'demo2', 'style' => 'demo2.css', 'html' => 'demo2.html', 'image' => 'demo2.jpeg', 'created_at' => $now, 'updated_at' => $now]);
            
        }

        //vxpt_pricing_tables

        $table_name = $wpdb->prefix . 'vxpt_pricing_tables';
        $charset_collate = $wpdb->get_charset_collate();
        if ($wpdb->get_var("show tables like '{$table_name}'") != $table_name) {
            $sql = "CREATE TABLE IF NOT EXISTS " . $table_name . " (
             `id` INT(11) NOT NULL AUTO_INCREMENT,
			 ...
			 `updated_at` DATETIME NOT NULL,
			  ...
			  FOREIGN KEY(template_id) 
                REFERENCES " . $wpdb->prefix . "vxpt_templates (id)
            )$charset_collate;";
                dbDelta($sql);
        }
        //vxpt_columns
        $table_name = $wpdb->prefix . 'vxpt_columns';
        $charset_collate = $wpdb->get_charset_collate();
        if ($wpdb->get_var("show tables like '{$table_name}'") != $table_name) {
            $sql = "CREATE TABLE IF NOT EXISTS " . $table_name . " (
             `id` INT(11) NOT NULL AUTO_INCREMENT,
			 ...
			 `updated_at` DATETIME NOT NULL,
			  ...
			  FOREIGN KEY(table_id) 
                REFERENCES " . $wpdb->prefix . "vxpt_pricing_tables (id)
                ON DELETE CASCADE
		    )$charset_collate;";
            dbDelta($sql);
        }
        ...
        // vxpt_features
        $table_name = $wpdb->prefix . 'vxpt_features';
        $charset_collate = $wpdb->get_charset_collate();
        if ($wpdb->get_var("show tables like '{$table_name}'") != $table_name) {
            $sql = "CREATE TABLE IF NOT EXISTS " . $table_name . " (
             `id` INT(11) NOT NULL AUTO_INCREMENT,
			 ...
			 `updated_at` DATETIME NOT NULL,
			  ...
			  FOREIGN KEY(column_id) 
                REFERENCES " . $wpdb->prefix . "vxpt_columns (id)
                ON DELETE CASCADE
            )$charset_collate;";
                dbDelta($sql);
        }
        ...
        $table_name = $wpdb->prefix . 'vxpt_currency';
        $charset_collate = $wpdb->get_charset_collate();
        if ($wpdb->get_var("show tables like '{$table_name}'") != $table_name) {
            $sql = "CREATE TABLE IF NOT EXISTS " . $table_name . " (
